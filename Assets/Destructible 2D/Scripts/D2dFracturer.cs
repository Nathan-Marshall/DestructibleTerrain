...
namespace Destructible2D
{
	...
	public class D2dFracturer : MonoBehaviour
	{
		...
		{
			return TryFracture(cachedDestructible, FinalPoints, splitAfterFracture, splitFeather);
		}
		...
		public static bool TryFracture(D2dDestructible destructible, int pointCount, bool splitAfterFracture, int splitFeather)
		{
				...
				if (voronoiCount > 1)
				{
					...
					if (splitAfterFracture == true)
					{
						for (var i = chunks.Count - 1; i >= 0; i--)
						{
							...
							if (chunk != null)
							{
								D2dSplitter.TrySplit(chunk, splitFeather);
							}
						}
					}
